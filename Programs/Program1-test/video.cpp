...
video::~video()
{
  ...
  date_to_watch = video_length = 0;
}

int video::add(int date_to_watch_to_add, char video_name_to_add[], char teacher_to_add[], char topic_to_add[], int video_length_to_add)
{
  if(!video_name_to_add || !teacher_to_add) return 0;

  date_to_watch = date_to_watch_to_add;
